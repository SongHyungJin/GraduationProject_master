...
ORIGINAL_IMAGE_PATH = r"image/갑옷.png"
OUTPUT_DIR = "generated_images"
os.makedirs(OUTPUT_DIR, exist_ok=True)
out_dir = OUTPUT_DIR

PROMPTBANK_PATH = r"promptbank/diffusiondb_prompts1.txt"
PROMPTBANK_TOPN = 80
...
FULL_W,  FULL_H,  FULL_EFF_STEPS  = 1024, 1024, 10
...
OBJ_DEFAULT_DESCRIPTORS = {
    # weapon / armor family used in our multi-object experiments
    "sword": [
        "silver blade",
        "wooden hilt",
        "ornate crossguard",
...
def _safe_swap_first_element(final_prompt: str, new_object: str):
    """
    Swap the first prompt element to `new_object` and,
    when possible, also replace old object–specific tokens
    (e.g. "cheek guard", "visor") with descriptors that match
    the new object (e.g. "silver blade" for a sword).

    The replacement is intentionally simple and local so that
    the rest of the pipeline behaviour is unchanged.
    """
    ...
    # Split into comma‑separated elements
    parts = [p.strip() for p in str(final_prompt).split(",")]
    if not parts:
        return new_object

    # Remember old main object before swap
    old_main = parts[0] if parts else ""
    parts[0] = new_object

    # If we cannot introspect helpers, just return the simple swap
    try:
        from math import inf  # dummy import to keep try block non‑empty
    except Exception:
        return ", ".join([p for p in parts if p])

    # Helper: head/category for old/new objects
    try:
        old_head = _map_to_head_category(_head_noun(old_main)) if old_main else ""
        new_head = _map_to_head_category(_head_noun(new_object)) if new_object else ""
    except Exception:
        old_head = ""
        new_head = ""

    # Build vocabulary that marks "old‑object‑specific" phrases
    old_part_phrases = []
    try:
        if old_head and isinstance(OBJ_PARTS, dict):
            for ph in OBJ_PARTS.get(old_head, []):
                ph = (ph or "").strip().lower()
                if ph:
                    old_part_phrases.append(ph)
    except Exception:
        pass

    old_synonyms = []
    try:
        if old_head and isinstance(OBJ_SYNONYM_MAP, dict):
            for ph in OBJ_SYNONYM_MAP.get(old_head, set()):
                ph = (ph or "").strip().lower()
                if ph:
                    old_part_phrases.append(ph)
                    old_synonyms.append(ph)
    except Exception:
        pass

    def _is_old_specific_token(tok: str) -> bool:
        t = (tok or "").strip().lower()
        if not t:
            return False
        # direct head‑noun match
        try:
            if old_head and _map_to_head_category(_head_noun(t)) == old_head:
                return True
        except Exception:
            pass
        # substring match against known parts/synonyms
        for ph in old_part_phrases:
            if ph and ph in t:
                return True
        return False

    # New object default descriptors (if available)
    new_descs = []
    try:
        if isinstance(OBJ_DEFAULT_DESCRIPTORS, dict) and new_head:
            new_descs = [d for d in OBJ_DEFAULT_DESCRIPTORS.get(new_head, []) if d]
    except Exception:
        new_descs = []

    # Replace old‑specific tokens with new descriptors, if we have any
    replaced_any = False
    if new_descs:
        desc_idx = 0
        for i in range(1, len(parts)):
            if _is_old_specific_token(parts[i]):
                if desc_idx < len(new_descs):
                    parts[i] = new_descs[desc_idx]
                    desc_idx += 1
                else:
                    # If we run out of descriptors, drop clearly old‑specific tokens
                    parts[i] = ""
                replaced_any = True

    # If nothing was replaced but we have descriptors, append at least one
    if (not replaced_any) and new_descs:
        # avoid duplicating if similar text already present
        first_desc = new_descs[0]
        already_has = any(first_desc.lower() in (p or "").lower() for p in parts)
        if not already_has:
...
if __name__ == "__main__":
    ...
    for i, e in enumerate(prompt_elements, 1):
        print(f"{i:02d}. {e}")
    k = 8 if len(prompt_elements) >= 8 else max(2, len(prompt_elements))
...
try:
    _swap_input = input("Swap #0? (n=keep, y=enter object, or type custom name): ").strip()
except Exception:
    _swap_input = "n"

use_cn_for_final = True
use_ipa_for_final = True

if _swap_input.lower() == 'n':
    pass
else:
    # If user explicitly chose 'y' (or pressed Enter), ask for the new object name.
    if _swap_input.lower() == 'y' or _swap_input.strip() == '':
        try:
            _new_obj = input("Enter new main object (e.g., 'sword', 'bow', 'helmet'): ").strip()
        except Exception:
            _new_obj = ""
    else:
        # Treat the first answer as a direct custom object name.
        _new_obj = _swap_input.strip()

    if _new_obj:
        final_prompt = _safe_swap_first_element(final_prompt, _new_obj)
        use_cn_for_final = False
        use_ipa_for_final = False
...
ok, clip, info = generate_final_image_with_toggle(
    ...
    use_cn=use_cn_for_final,
    use_ipa=use_ipa_for_final,
